...
export function buildRecommendationNarrative(input: NarrativeInput): string {
  ...
  if (winnerId === 'dividends-only') {
    ...
    return `With your corporate balance and investment return, avoiding CPP premiums and RRSP complexity ${estateMsg} — the simpler dividend structure wins here. Projected retirement income: ${income}/yr with a ${successPct}% success rate.`;
  }
  ...
  if (winnerId === 'salary-at-ympe') {
    ...
    return `Maximizing CPP through a salary at YMPE builds guaranteed, inflation-indexed retirement income that reduces your reliance on corporate drawdown. This produces ${income}/yr in sustainable retirement spending (${successPct}% success rate) while generating ${rrspMsg}.`;
  }
